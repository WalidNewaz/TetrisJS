...
export default class Board {
    ...
    isLineComplete() {
        ...
        if (lines.length > 0) {
            ...
            if (lines.length == 4) {
                this.score.textContent = parseInt(this.score.textContent) + fixedScores[4];
                // tetrisCount++;
                incTetrisCount();
                if (tetrisCount == levelTargets.level1.tcount && currentLevel == 1) {
                    // gameSpeed = levels[1];
                    setGameSpeed(levels[1]);
                    this.levelElement.textContent = parseInt(this.levelElement.textContent) + 1;
                    // currentLevel++;
                    incCurrentLevel();
                    // levelIncreased = true;
                ...
                this.score.textContent = parseInt(this.score.textContent) + fixedScores[lines.length];
            }

            //changing level based on score
            let intScore = parseInt(this.score.textContent);
            ...
            //game won
            if ((tetrisCount >= levelTargets.level4.tcount || parseInt(this.score.textContent) >= levelTargets.level4.score) && currentLevel == 4) {
                // isGameWon = true;
                setIsGameWon(true);
            }
    ...
    isGameFinished() {
        ...
        if (sum > 0) {
            // isGameOver = true;
            setIsGameOver(true);setIsGameOver
        }
    }
}