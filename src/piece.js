...
export function incCurrentLevel(val) {
    currentLevel++;
}
    ...
    draw() {
        ...
        this.shape.forEach((row, y) => {
            row.forEach((value, x) => {
                if (value > 0) {
                    //well[y][x]=value;                    
                    this.ctx.fillRect(this.x * (singleBlockSize) + x * (singleBlockSize), this.y * (singleBlockSize) + y * (singleBlockSize), blockActualParam, blockActualParam);
                }
            })
        });
    }
